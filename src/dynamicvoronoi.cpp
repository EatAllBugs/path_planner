...
void DynamicVoronoi::reviveVoroNeighbors(int &x, int &y) {
  for (int dx = -1; dx <= 1; dx++) {
    ...
    for (int dy = -1; dy <= 1; dy++) {
      ...
      dataCell nc = data[nx][ny];
      if (nc.sqdist != INT_MAX && !nc.needsRaise && (nc.voronoi == voronoiKeep ||
          nc.voronoi == voronoiPrune)) {
        nc.voronoi = free;
        data[nx][ny] = nc;
        pruneQueue.push(INTPOINT(nx, ny));
      }
...
void DynamicVoronoi::prune() {
  ...
  while (!open.empty()) {
    ...
    int v = c.voronoi;
    if (v != freeQueued &&
      v != voronoiRetry) { // || v>free || v==voronoiPrune || v==voronoiKeep) {
      //      assert(v!=retry);
      continue;
    }
...
DynamicVoronoi::markerMatchResult DynamicVoronoi::markerMatch(int x, int y) {
  ...
  // 4-connected
  if ((!f[0] && f[1] && f[3]) || (!f[2] && f[1] && f[4]) || (!f[5] && f[3] &&
      f[6]) || (!f[7] && f[6] && f[4])) {
    return keep;
  }
